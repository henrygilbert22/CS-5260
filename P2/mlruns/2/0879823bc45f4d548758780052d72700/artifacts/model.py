...
class Model():
    ...
    def train(self, replay_memory: deque, done: bool) -> None:
        ...
        data_set_size = 1000     # Getting random 128 batch sample from 
        ...
        [self.big_X.append(x) for x in X]
        [self.big_Y.append(y) for y in Y]

        if len(self.big_X) > 5000:
            with open('big_X.pkl', 'wb') as f:
                pickle.dump(self.big_X, f)
            with open('big_Y.pkl', 'wb') as f:
                pickle.dump(self.big_Y, f)
            sys.exit(1)
        ...
        #history = self.model.fit(train_data, batch_size=16, verbose=2, shuffle=True, epochs=1)      
