...
class Model():
    ...
    def train(self, replay_memory: deque, done: bool) -> None:
        ...
        current_states = np.array(current_states).reshape([len(current_states[0]), np.array(current_states).shape[0]])
        current_qs_list = self.model(current_states)     # Predict the q values based on all the historical state
        ...
        new_current_states = np.array(new_current_states).reshape([len(new_current_states[0]), np.array(new_current_states).shape[0]])
        future_qs_list = self.target_model(new_current_states)       # the q values resulting in our action
        ...
        history = self.model.fit(self.create_dataset((X,Y)))  
        return 0, 0
