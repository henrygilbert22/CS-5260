...
class Model():
    ...
    def train(self, replay_memory: deque, done: bool) -> None:
        ...
        current_states = np.array(current_states).reshape([len(current_states), np.array(current_states).shape[0]])
        ...
        new_current_states = np.array(new_current_states).reshape([len(new_current_states), np.array(new_current_states).shape[0]])
