...
class Model():
    ...
    def step(self, environment: Environment) -> int:
        ...
        if done and len(self.replay_memory) > 500:
            t = Thread(target=self.train, args=(True,))
            t.start()
            self.episode += 1
            self.epsilon = self.min_epsilon + (self.max_epsilon - self.min_epsilon) * np.exp(-0.01 * self.episode)
            self.update_target(self.model)

        elif self.steps_taken % 10 == 0 and len(self.replay_memory) > 500:
            t = Thread(target=self.train, args=(True,))
            t.start()
    ...
    def train(self, done: bool) -> None:
        ...
        print(f"Training: {self.country_index}")
        MIN_REPLAY_SIZE = 500      
