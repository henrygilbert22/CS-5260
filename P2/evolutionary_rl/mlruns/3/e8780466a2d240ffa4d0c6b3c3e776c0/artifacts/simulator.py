...
def simulate_single_country(country_index: int, shared_total_rewards: list, rewards: list, shared_weights: list):

        env = Environment()
        model = Model((5, 15), 7, country_index)
        waiting_for_model = lambda rewards: [reward for reward in rewards if not reward]
