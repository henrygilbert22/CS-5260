...
class Model():
    ...
    def step(self, environment: Environment) -> int:
        ...
        if done:
            p = mp.Process(target=self.train, args=(done,))
            p.start()
            self.episode += 1
    ...
    def update_target(self, model_to_copy: keras.Sequential) -> None:
        self.target_model.set_weights(model_to_copy.get_weights())

def train(done: bool, model: keras.Sequential, target_model: keras.Sequential, replay_memory: deque, country_index: int, shared_list: list) -> None:
    ...
    current_qs_list = model(current_states).numpy()     # Predict the q values based on all the historical state
    ...
    future_qs_list = target_model(new_current_states).numpy()       # the q values resulting in our action
    ...
    history = model.fit(train_data, verbose=1, shuffle=False) 
    shared_list.append([country_index, model])
