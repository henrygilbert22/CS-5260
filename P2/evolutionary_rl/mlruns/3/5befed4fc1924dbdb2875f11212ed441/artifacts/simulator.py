...
def simulate_single_country(country_index: int, shared_total_rewards: list, shared_weights: list):
        ...
        for i in tqdm(range(10)):
            ...
            print(f'{country_index} Got: {model.total_reward}')
            ...
            if np.argmax(shared_total_rewards) == country_index:
                ...
                print(f"{country_index} was the best model")
...
def temp(results: list):
    ...
    s = Simulator()
    results.append(s.simulate_parallel())
...
def test():
    ...
    for i in range(5):
        pool.apply_async(temp, args=(results,))
